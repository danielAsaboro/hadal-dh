...
app = typer.Typer(no_args_is_help=True, help="Know what breaks before you merge.")


@app.callback()
def main() -> None:
    """Run impact-aware pull-request checks."""
...
def review(
    ...
    output: Path = typer.Option(Path("cutset-output"), help="Report directory."),
    write_back: bool = typer.Option(False, help="Persist the report and risk tag in DataHub."),
    tag_name: str = typer.Option("cutset-at-risk", help="Existing DataHub risk tag name."),
) -> None:
    ...
    try:
        ...
        typer.echo(f"Cutset input error: {error}", err=True)
        raise typer.Exit(2) from error
    except DataHubContextError as error:
        typer.echo(f"Cutset context error: {error}", err=True)
