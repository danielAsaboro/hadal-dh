...
def test_normalizes_entity_governance_and_health() -> None:
    ...
    assert contexts[0].incident_statuses == ("PASS",)
...
def test_removes_sql_comments_before_query_enters_evidence() -> None:
    _, queries = normalize_queries(
        DATASET,
        {
            "total": 1,
            "start": 0,
            ...
            "queries": [
                {
                    ...
                    "properties": {
                        "statement": {
                            "value": (
                                "/* customer secret NG-447 */ select email "
                                "from analytics.customers -- private export"
                            ),
                            "language": "SQL",
                        },
                        "source": "SYSTEM",
    ...
    assert "NG-447" not in queries[0].statement
    assert "private export" not in queries[0].statement
    assert queries[0].statement == "SELECT email FROM analytics.customers"
...
def test_normalizes_never_run_assertion_as_unknown() -> None:
    sample = {
        "success": True,
        "data": {
            "start": 0,
            ...
            "total": 1,
            "assertions": [
                {"urn": "urn:li:assertion:new-check", "type": "FIELD"}
            ],
        },
    }
    empty = {
    ...
    summary = normalize_assertions(sample, empty, empty)

    assert summary.sample[0].status == "UNKNOWN"


def test_rejects_filtered_assertion_total_above_verified_total() -> None:
    sample = {
        "success": True,
        "data": {"start": 0, "count": 0, "total": 1, "assertions": []},
    }
    failing = {
        "success": True,
        "data": {"start": 0, "count": 0, "total": 2, "assertions": []},
    }
    empty = {
        "success": True,
        "data": {"start": 0, "count": 0, "total": 0, "assertions": []},
    ...
    with pytest.raises(ContextNormalizationError, match="exceed"):
        normalize_assertions(sample, failing, empty)
